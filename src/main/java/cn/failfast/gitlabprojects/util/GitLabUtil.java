package cn.failfast.gitlabprojects.util;
...
public class GitLabUtil {
    ...
    public static Pair<GitRemote, String> findGitLabRemote(@NotNull GitRepository repository) {
        for (GitRemote gitRemote : repository.getRemotes()) {
            for (String remoteUrl : gitRemote.getUrls()) {
                GitlabServer gitlabServer = settingsState.currentGitlabServer(repository);
                if (gitlabServer == null) {
                    return null;
                }
                if (remoteUrl.contains(gitlabServer.getRepositoryUrl())) {
    ...
    public static boolean testGitExecutable(final Project project) {
        ...
        if (!version.isSupported()) {
            MessageUtil.showErrorDialog(project, "Your version of git is not supported.", "Cannot Find Git");
            return false;
        }
        return true;
    }
